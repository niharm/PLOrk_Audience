...
function Noise(xAccel, yAccel, zAccel) {
    ...
    var decayTime = Math.max(yAccel / 5.0, 0.2);
    var filterFreq = Math.min(xAccel * 300.0 + 500.0, 7000);
    var endFilterFreq = Math.max(filterFreq + getRandomInInterval(-2000, 2000), 500);
    var randomQ = getRandomInInterval(1, 5); 
    ...
    noiseFilter.Q.setValueAtTime(randomQ, context.currentTime);
    noiseFilter.Q.setTargetAtTime(0.0, context.currentTime, decayTime);
